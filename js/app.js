...
<br>
<footer class="footer mt-auto py-3 text-muted ${UI.footer_style_class}" style="${UI.fixed_footer ?'position: fixed; ': ''}left: 0; bottom: 0; width: 100%; color: white; z-index: 9999;${UI.hide_footer ? ' display:none;': ' display:block;'}"> <div class="container" style="width: auto; padding: 0 10px;"> ${UI.credit ? '<p>Redesigned with <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-heart-fill" fill="red" xmlns="http://www.w3.org/2000/svg"> <path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z" /> </svg> by <a href="https://www.npmjs.com/package/@googledrive/index" target="_blank">TheFirstSpeedster</a>, based on Open Source Softwares.</p>' : ''} <p style="text-align: center;"> ${UI.reload_text}  </p> <br> <p style="text-align: center;"> <a href=" ${UI.credits_link}" style="color:${UI.credits_color}; text-decoration:none"> ${UI.credits_text} </a>  &nbsp;&nbsp; <a href=" ${UI.company_link}" target="_blank" style="text-decoration:none"> ${UI.company_name} </a> ${UI.copyright_year} </p></div></footer>
  `;
    $('body').html(html);
}
